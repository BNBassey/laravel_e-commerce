<x-admin-layout>
    ...
    <main class="flex-1 overflow-y-auto p-6 bg-gray-100">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            ...
            <a href="{{ route('admin.coupon.add') }}" class="bg-primary hover:bg-blue-600 text-white px-5 py-2.5 rounded-lg text-sm font-medium transition flex items-center gap-2 shadow-sm">
                        ...
                        @forelse ($coupons as $coupon )
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <span class="bg-purple-50 text-purple-700 px-2 py-1 rounded text-xs font-semibold uppercase">{{ strtoupper($coupon->type) }}</span>
                                    @else
                                    <span class="bg-blue-50 text-blue-700 px-2 py-1 rounded text-xs font-semibold uppercase">{{ strtoupper($coupon->type) }}</span>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-600"><i class="fa-regular fa-calendar mr-1"></i> {{ \Carbon\Carbon::parse($coupon->expiry_date)->format('d-m-Y') }}</span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.coupon.edit', ['coupon_id'=>$coupon->id]) }}" class="w-8 h-8 rounded-full hover:bg-gray-100 text-blue-500 transition flex items-center justify-center" title="Edit">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                        <form id="delete-form-{{ $coupon->id }}" action="{{ route('admin.coupon.delete', ['coupon_id'=>$coupon->id]) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="w-8 h-8 rounded-full hover:bg-gray-100 text-red-500 transition flex items-center justify-center" onclick="deleteCoupon(this, '{{ $coupon->code }}', {{ $coupon->id }})" title="Delete">
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center text-gray-500">
                                        ...
                                        <a href="{{ route('admin.coupon.add') }}" class="mt-4 text-primary hover:underline text-sm font-medium">
